...
if __name__ == '__main__':
    try:
        ...
        pass  # Already set or context prevents changing

# --- Imports (AFTER setting start method) ---
# C++ Kernel: Contains OPPObjective and Solvers
...
# ==============================================================================
# OPP EXPERIMENT CONFIGURATION
# ==============================================================================

# 1. Experiment Parameters
D_DIMENSION = 10  # d: Dimension of the orthogonal group O(d) (n=k=d)
NUM_PARTICLES = 50  # N: Number of particles
# Note: Sigma might need tuning for OPP. Start with a moderate value.
SIGMA_VAL = 0.2  # sigma: Noise coefficient (adjust as needed)
T_VAL = 100.0  # T: Total simulation time (adjust as needed)
DT_VAL = 0.01  # dt: Time step
NUM_TRIALS = 100  # Number of independent trials

# 2. Success Tolerance
# We will check if the result is close to the known analytical minimum.
SUCCESS_TOLERANCE_ABS = 0.1  # Absolute error from true minimum for success

# 3. Solver Parameter Dictionaries
KIM_SOLVER_PARAMS = {
    'beta_val': 100.0,  # Beta often lower for quadratic objectives
    'lambda_val': 1.0,
...
def setup_and_run_opp_experiment(solver_class, backend):
    """
    Sets up and runs the OPP experiment case O(d) = V(d, d).
    """
    ...
    OBJECTIVE_NAME_CPP = "OPP"  # String identifier for C++ objective

    print("=" * 60)
    print(f"--- STARTING OPP SUITE: {solver_class.__name__} (C++) ---")
    ...
    temp_obj = cbo_module.OPPObjective(D_DIMENSION)
    true_min_for_reporting = temp_obj.get_true_minimum()
    print(f"[INFO] True Minimum for OPP: {true_min_for_reporting:.4f}")

    # Define success as being close to this known minimum
    ...
    runner = ExperimentRunner(
        ...
        dt=DT_VAL,
        trials=NUM_TRIALS,
        success_tolerance=calculated_success_tolerance,
...
if __name__ == '__main__':
    ...
    opp_runner = setup_and_run_opp_experiment(SOLVER_TO_RUN, ACCELERATION_BACKEND)

    # --- 3. PLOT RESULTS ---
    time_pts, dynamics, asymptotics, obj_instance = opp_runner.get_results_for_plotting()
    ...
    print("\n--- OPP experiment suite execution finished. ---")